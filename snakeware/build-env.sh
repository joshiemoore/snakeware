...
if [ ! -d "$BUILDDIR" ]; then
	...
	"$ROOT_COMMAND" env DEBOOTSTRAP_DIR="$WORKDIR/usr/share/debootstrap/" \
		...
		--include "bc,ca-certificates,cpio,fakeroot,file,git,kpartx,libelf-dev,libglib2.0-0,libpython3.8,libssl-dev,libncurses-dev,python3.8,rsync,sudo,unzip,wget" \
